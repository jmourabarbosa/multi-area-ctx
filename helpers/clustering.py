...
# def gmm_bic(vectors, max_components=5, algo="bayes", n_init=50, mean_precision_prior=1e-4):
#     """
#     fit a mixture of gaussians to a set of vectors for components ranging from 1 to 10 and plot the BIC curve
#     :param vectors: list of numpy arrays of shape n
#     :param max_components: int
#     :param algo: 'em' or 'bayes'
#     :param n_init: number of random seeds for the inference algorithm
#     :return: best number of components, best fitted model
#     """
#     X = np.vstack(vectors).transpose()
...
#         if algo == "em":
#             model = GaussianMixture(n_components=p, n_init=n_init)
#         else:
#             model = BayesianGaussianMixture(n_components=p, n_init=n_init, mean_precision_prior=mean_precision_prior)
#         model.fit(X)
#         score = model.score(X)
#         scores.append(score)
#         print('{} components, score={:.3e}'.format(p, score))
#         n_params = (p * d) + (p * d * (d + 1) / 2) + (p - 1)  # number of parameters of the mixture
#         bic = n_params * np.log(X.shape[0]) - 2 * score * X.shape[0]
#         print('{} components, bic={:.3e}'.format(p, bic))
#         bics.append(bic)
#         if bic < best_bic:
#             best_bic = bic
#             best_model = model
#             best_p = p
#     plt.plot(range(1, max_components+1), bics)
...
def pop_scatter_linreg(vec1, vec2, pops, n_pops=None, linreg=True, colors=('blue', 'green', 'red', 'violet', 'gray'), size=10.,
                       figsize=(5, 5), ax=None):
    """
